...
import { selectShowDialog, setShowDialog } from "@/store/uiSlice";
...
export default function SubscribeDialog() {
  ...
  const { t } = useTranslation(["coming_soon", "buttons", "forms"]);
  const dispatch = useDispatch();
  const shouldShowDialog = useAppSelector(selectShowDialog);

  const [hasError, setError] = useState(false);
  const [email, setEmail] = useState("");
  const [isLoading, setIsLoading] = useState(false);

  const handleOpen = () => dispatch(setShowDialog(!shouldShowDialog));
      ...
      <Card className="mx-auto w-full max-w-[24rem]">
        ...
        <CardBody className="flex flex-col gap-4">
          <div className="flex flex-col gap-4 items-center">
            <Image width="70" height="70" alt="img" src="/logo-black.png" />
            <Typography variant="h4" color="blue-gray">
              {t("welcome")}
            </Typography>
            <Typography
              className="mb-3 text-center font-normal"
              variant="paragraph"
              color="gray"
            >
              {t("description")}
          ...
          />
          {hasError ? (
            <div className="text-center text-red-400 md:text-md text-sm font-semibold">
              {t("error")}
            </div>
          ) : null}
        </CardBody>
